...
public sealed class LucideGeneratorRunner : ILucideGeneratorRunner
{
    ...
    public async ValueTask<int> Run(CancellationToken cancellationToken = default)
    {
        ...
        string interfacePath = Path.Combine(outputDir ?? Directory.GetCurrentDirectory(), "ILucideIconSvgProvider.g.cs");
        string interfaceContent = GenerateILucideIconSvgProvider();
        await File.WriteAllTextAsync(interfacePath, interfaceContent, cancellationToken).ConfigureAwait(false);
        _logger.LogInformation("Generated {InterfacePath}.", interfacePath);
    ...
    private static string GenerateILucideIconSvgProvider()
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        sb.AppendLine("#nullable enable");
        ...
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Provides SVG markup for Lucide icons by name.");
        sb.AppendLine("/// Implement and register when using the <see cref=\"Lucide\"/> element with Soenneker.Quark.Gen.Lucide.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("public interface ILucideIconSvgProvider");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Returns the SVG markup for the given Lucide icon name (PascalCase), or null if not found.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    string? GetSvg(string iconName);");
        sb.AppendLine("}");
        return sb.ToString();
    }
